...
function changePlayer() {
    ...
    //Is player 2 turn 
        ...
        //
        //gets the element of playerOneHealth
        let playerOneHealth = document.getElementById("playerOneHealth");
        
        //turns the playerOneHealthNum from a strong into a number
        let playerOneHealthNum = Number(playerOneHealth.innerHTML);
        //this line takes 10pts of playerOneHealth
        playerOneHealthNum -= 10;
        //This line updates the playerOneHealthNum to the new value
        playerOneHealth.innerHTML = playerOneHealthNum;
        //these lines of code check if playerOneHealthNum is less or equal to zero then the game is over. 
        ...
        //switches to the next player, and changes the UI behavior
        gameState.whoseTurn = 1;
        //gets the player's name element and changes the players turn that is displayed.
        let playerName = document.getElementById("playerName");
        playerName.innerHTML = `Player ${gameState.whoseTurn}`;
...
function attackPlayerOne() {
    ...
    //this function switches the player 2 attack button to active and player one to inactive, so player two can fight against player one,this disables the button of player one.
    function changeButtonStatus() {
        let playerTwoAttackButton = document.getElementById("playerTwoAttack");
    ...
    //this function changes player two sprites with the array of frames 
    function animatePlayer() {
        // an array containing the images using in player one's animation
        // the indices are later used to cycle / "animate" when the player attacks
        let playerTwoFrames = [
        ...
        //this code is grabbing using the DOM method playerTwoSprite
        let playerSprite = document.getElementById("playerTwoSprite");
        //This is selecting out of the array of frames, the attack frame for player 2
        playerSprite.src = playerTwoFrames[1];
        
        // removes the idle class in the playerSprite, so player 2 can fight.
        playerSprite.classList.remove("idle");
        
        //ask the attack class to the player sprite of player 2
        playerSprite.classList.add("playerTwoAttack");

        // gets the enemy sprite by using DOM method
        let enemySprite = document.getElementById("playerOneSprite");
        //plays a song when player two is attacking player one
        let enemyDamage = document.getElementById("SFX_EnemyDamage");
        // Removes the idle class from the enemySprite
        enemySprite.classList.remove("idle");
        //adds the playerOne damage to player 1
        enemySprite.classList.add("playerOneDamage");
        // sound that plays when enemy takes damage
        enemyDamage.play();

        //thsi function after 350 milisec will run the code below
        function changePlayerTwoSprite() {
            //removes playerOneDamage from player 1 
            enemySprite.classList.remove("playerOneDamage");
            //adds the idle class to the enemySprite
            enemySprite.classList.add("idle");
            playerSprite.src = playerTwoFrames[0];
            //removes the playerTwoAttack class
            playerSprite.classList.remove("playerTwoAttack");
            //adds the idle class that is when player two is resting not fighting
            playerSprite.classList.add("idle");
        }
//This setTimeout method will call the function changePlayerTwoSprite after 350 miliseconds
        setTimeout(changePlayerTwoSprite, 350);
        
    }
    //This function check if is player number 2 turn and if it is calls a bunch of other functions to continue the game
